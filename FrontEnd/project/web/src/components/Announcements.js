...
function Announcements(props) {
  ...
  const getData1 = async () => {
    try {
      const response = await fetch(
        "http://localhost:8080/class_announcements/list"
      );
      if (!response.ok) {
        throw new Error(
      ...
      for (var i = 0; i < length; i++) {
        ...
        console.log(i);
        if (
          new String(courseId).valueOf() ===
          new String(actualData.data[i].class_id).valueOf()
        ) {
          console.log(i);
  ...
  const getData2 = async () => {
    try {
      ...
      var output = {
        announcementTitles: tempAnnouncementTitles,
        announcements: tempAnnouncements,
        announcementDates: tempAnnouncementDates,
      };
